...
function readPkg () {
    try {
        var pkg = JSON.parse(readFile('package.json').toString());
        // if (!pkg || typeof pkg !== 'object')
        //     throw new TypeError('package.json does not specify an Object');
    }
    catch (err) {
...
function readPkgName () {
    var name = readPkg().name;

    if (!name)
        throwError('Name is not specified in package.json.');

    return name;
}

function readPkgVersion () {
    var version = readPkg().version;
...
function publish (opts) {
    var command = 'npm publish --tag ' + opts.tag;

    if (isScopedPackage(readPkgName()))
        command += ' --access ' + opts.scopeAccess;
...
function getOptions (opts) {
    ...
    var scopeAccess = isScopedPackage(readPkgName()) ? 'restricted' : 'public';
